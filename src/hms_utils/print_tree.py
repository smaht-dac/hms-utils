...
def print_tree(data: dict, indent: Optional[int] = None,
               hide_values: bool = False, printf: Optional[Callable] = None) -> None:
    ...
    def traverse(data: dict, indent: str = "", first: bool = False, last: bool = True):  # noqa
        ...
        for index, key in enumerate(keys := list(data.keys())):
            ...
            if isinstance(value := data[key], dict):
                ...
                if hide_values is not False:
                    output(indent + corner + f"{key}: ********")
                else:
