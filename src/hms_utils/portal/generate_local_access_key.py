...
def main() -> None:
    ...
    parser.add_argument("--user", required=False,
                        help=f"User email for which the access-key"
                             f" should be defined (in master-inserts/user.json); or a UUID.")
    ...
    parser.add_argument("--update-keys", action="store_true", required=False, default=False,
                        help=f"Updates your access-keys file (e.g. ~/.smaht-keys.json)"
                             f" with the new access-key (e.g. smaht-local).")
    ...
    access_key_inserts_file_item = _generate_access_key_inserts_item(access_key_id,
                                                                     access_key_secret_hash, access_key_user_uuid)
    ...
    if args.update_database:
        if not _is_local_portal_running(args.port):
            _exit_without_action(f"Portal must be running"
                                 f" locally ({_get_local_portal_url(args.port)}) to do an insert.")
...
def _hash_secret_like_snovault(secret: str, ini_file: str = _DEFAULT_INI_FILE) -> str:
    ...
    def get_passlib_properties_from_ini_file(ini_file: str = _DEFAULT_INI_FILE,
                                             section_name: str = "app:app",
                                             property_name_prefix: str = "passlib.") -> str:
        """
